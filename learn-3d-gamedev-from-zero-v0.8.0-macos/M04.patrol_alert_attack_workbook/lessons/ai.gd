...
class StateMachine extends Node:
			...
			for state: State in transitions:
				...
				for event: Events in transitions[state]:
					assert(
						...
						"Expected an Events object, bot got " + str(event)
					)
					assert(
						transitions[state][event] is State,
						"Invalid state in the tranisitions dictionary. " +
	...
	func _on_state_finished(finished_state: State) -> void:
		assert(
			...
			"Recieved a state that does not have a transition for the FINISHED event, " + current_state.name + ". " +
